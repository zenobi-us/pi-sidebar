...
## Acceptance Criteria
- [ ] Provide an API for opening/closing the drawer with placement (left/right), width, and responsive visibility options.
- [ ] Support focus/input routing for drawer content, including IME-safe focus propagation guidelines.
- [ ] Define integration hooks for other extensions (commands/tools or shared state) to push content.
- [ ] Document constraints (overlay lifecycle, line width limits, visibility handling).
- [ ] Include example usage snippet demonstrating a right-side drawer and toggle behavior.
